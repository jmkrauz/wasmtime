...
fn apply_reloc(
    ...
    match (r.kind(), r.encoding(), r.size()) {
        ...
        (RelocationKind::Absolute, RelocationEncoding::Generic, 32) => unsafe {
            let reloc_address = body.add(offset as usize) as usize;
            let reloc_addend = r.addend() as isize;
        ...
        (RelocationKind::Elf(elf::R_ARM_THM_PC22), RelocationEncoding::Generic, 32) => unsafe {
            let reloc_address = body.add(offset as usize) as usize;
            let reloc_addend = r.addend() as isize;
            ...
            let s = (delta_bits >> 23) & 0x1;
            let i1 = (delta_bits >> 22) & 0x1;
            let i2 = (delta_bits >> 21) & 0x1;
            let j1 = (i1 ^ s) ^ 0x1;
            let j2 = (i2 ^ s) ^ 0x1;
